...
--- mark the current node as a "death ending"
--- engine-wise identical to is_end() — node is registered as End node and route terminates,
--- the "death" distinction is made at the FlowChartSlot level (slot.isDeath = true), which
--- drives the special slot visual + reroutes the player back to the flowchart instead of title.
--- use is_dead() in scenarios that end in player death; use is_end() for normal endings.
function is_dead()
    if not check_eager('is_dead') then
        return
    end
    __Nova.scriptLoader:SetCurrentAsEnd()
end
