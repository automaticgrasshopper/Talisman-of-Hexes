// This source code was auto-generated by ToLua#, do not modify it
using System;
using LuaInterface;

public class Nova_ScreenCapturerWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(Nova.ScreenCapturer), typeof(UnityEngine.MonoBehaviour));
		L.RegFunction("GetGameTexture", GetGameTexture);
		L.RegFunction("CaptureGameTexture", CaptureGameTexture);
		L.RegFunction("GetBookmarkThumbnailTexture", GetBookmarkThumbnailTexture);
		L.RegFunction("__eq", op_Equality);
		L.RegFunction("__tostring", ToLua.op_ToString);
		L.RegVar("capturedGameTexture", get_capturedGameTexture, set_capturedGameTexture);
		L.EndClass();
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetGameTexture(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 0)
			{
				UnityEngine.RenderTexture o = Nova.ScreenCapturer.GetGameTexture();
				ToLua.Push(L, o);
				return 1;
			}
			else if (count == 1)
			{
				UnityEngine.RenderTexture arg0 = (UnityEngine.RenderTexture)ToLua.CheckObject<UnityEngine.RenderTexture>(L, 1);
				UnityEngine.RenderTexture o = Nova.ScreenCapturer.GetGameTexture(arg0);
				ToLua.Push(L, o);
				return 1;
			}
			else if (count == 2)
			{
				UnityEngine.RenderTexture arg0 = (UnityEngine.RenderTexture)ToLua.CheckObject<UnityEngine.RenderTexture>(L, 1);
				UnityEngine.Camera arg1 = (UnityEngine.Camera)ToLua.CheckObject(L, 2, typeof(UnityEngine.Camera));
				UnityEngine.RenderTexture o = Nova.ScreenCapturer.GetGameTexture(arg0, arg1);
				ToLua.Push(L, o);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: Nova.ScreenCapturer.GetGameTexture");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int CaptureGameTexture(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			Nova.ScreenCapturer obj = (Nova.ScreenCapturer)ToLua.CheckObject<Nova.ScreenCapturer>(L, 1);
			UnityEngine.Camera arg0 = (UnityEngine.Camera)ToLua.CheckObject(L, 2, typeof(UnityEngine.Camera));
			obj.CaptureGameTexture(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int GetBookmarkThumbnailTexture(IntPtr L)
	{
		try
		{
			int count = LuaDLL.lua_gettop(L);

			if (count == 0)
			{
				UnityEngine.Texture2D o = Nova.ScreenCapturer.GetBookmarkThumbnailTexture();
				ToLua.PushSealed(L, o);
				return 1;
			}
			else if (count == 1)
			{
				UnityEngine.Material arg0 = (UnityEngine.Material)ToLua.CheckObject<UnityEngine.Material>(L, 1);
				UnityEngine.Texture2D o = Nova.ScreenCapturer.GetBookmarkThumbnailTexture(arg0);
				ToLua.PushSealed(L, o);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to method: Nova.ScreenCapturer.GetBookmarkThumbnailTexture");
			}
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int op_Equality(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			UnityEngine.Object arg0 = (UnityEngine.Object)ToLua.ToObject(L, 1);
			UnityEngine.Object arg1 = (UnityEngine.Object)ToLua.ToObject(L, 2);
			bool o = arg0 == arg1;
			LuaDLL.lua_pushboolean(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_capturedGameTexture(IntPtr L)
	{
		object o = null;

		try
		{
			o = ToLua.ToObject(L, 1);
			Nova.ScreenCapturer obj = (Nova.ScreenCapturer)o;
			UnityEngine.RenderTexture ret = obj.capturedGameTexture;
			ToLua.Push(L, ret);
			return 1;
		}
		catch(Exception e)
		{
			return LuaDLL.toluaL_exception(L, e, o, "attempt to index capturedGameTexture on a nil value");
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_capturedGameTexture(IntPtr L)
	{
		object o = null;

		try
		{
			o = ToLua.ToObject(L, 1);
			Nova.ScreenCapturer obj = (Nova.ScreenCapturer)o;
			UnityEngine.RenderTexture arg0 = (UnityEngine.RenderTexture)ToLua.CheckObject<UnityEngine.RenderTexture>(L, 2);
			obj.capturedGameTexture = arg0;
			return 0;
		}
		catch(Exception e)
		{
			return LuaDLL.toluaL_exception(L, e, o, "attempt to index capturedGameTexture on a nil value");
		}
	}
}

