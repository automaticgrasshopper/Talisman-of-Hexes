...
namespace Nova
{
    ...
    public class LuaRuntime : MonoBehaviour
    {
        ...
        // Subtitle data lives in Resources/Subtitles/subtitles_<locale>.txt as plain
        // Lua source assigning into the _subtitle_data global declared in requires.lua.
        // Loaded after requires.lua so framework + content stay decoupled.
        private void LoadSubtitleData()
        {
            var assets = Resources.LoadAll<TextAsset>("Subtitles");
            foreach (var asset in assets)
            {
                lua.DoString(asset.text, asset.name);
