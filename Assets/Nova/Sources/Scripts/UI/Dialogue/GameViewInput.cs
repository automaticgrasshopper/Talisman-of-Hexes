...
namespace Nova
{
    ...
    public class GameViewInput : MonoBehaviour, IRestorable, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private AudioClip clickSound; // 点击音效（通过 viewManager.TryPlaySound 走系统 UI 音效通道）
        ...
        // Lazy lookup for VideoController: it lives on a separate prefab whose Awake order
        // is not guaranteed relative to this one. Used to gate ShowLog while a video is playing.
        private VideoController videoController;
        private bool videoControllerSearched;

        private bool IsVideoPlaying()
        {
            if (!videoControllerSearched)
            {
                videoControllerSearched = true;
        ...
        // [Deprecated] 点击音效已并入系统 UI 音效通道（viewManager.TryPlaySound），
        // 音量由系统音效总开关统一管理。下面两个方法保留为空桩，仅为兼容旧 ToLua wrap
        // (Nova_GameViewInputWrap.cs)；可在重新生成 ToLua wrap 时一起删掉。
        public void SetClickSoundVolume(float volume) { /* no-op: 走系统音效音量 */ }

        public float GetClickSoundVolume() { return 1f; }
        ...
        public void OnPointerDown(PointerEventData _eventData)
        {
            ...
            // ButtonRing 已被 SideMenu 替代，触发逻辑暂时注释保留（可回滚）。
        ...
        public void OnPointerUp(PointerEventData _eventData)
        {
            ...
            if (eventData.button == PointerEventData.InputButton.Right)
            {
                ...
                // ButtonRing 已被 SideMenu 替代，右键 ShowButtonRing 分支暂时注释保留（可回滚）。
        ...
        private void ClickForward()
        {
            ...
            // 如果不是快进模式，通过系统 UI 音效通道播放点击音
            // （音量由系统音效总控管理，跟其它 UI 音效保持一致）
            if (!dialogueState.isFastForward && clickSound != null)
            {
                viewManager.TryPlaySound(clickSound);
            }
