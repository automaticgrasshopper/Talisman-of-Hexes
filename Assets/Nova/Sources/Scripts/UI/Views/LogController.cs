...
namespace Nova
{
    public class LogEntry
    {
        ...
        public readonly string subtitleText; // non-null marks this as a video-subtitle entry

        public bool isSubtitle => subtitleText != null;
        ...
        public LogEntry(float height, float prefixHeight, string subtitleText)
        {
            this.height = height;
            this.prefixHeight = prefixHeight;
            this.nodeOffset = -1;
        ...
        LoopScrollSizeHelper
    {
        ...
        // Subtitle log entries are appended in event order alongside dialogue entries.
        // They are rendered grey (via TMP rich-text) and never restored from save data.
        public void AddSubtitleEntry(string text)
        {
            if (string.IsNullOrEmpty(text))
            {
                return;
            ...
            var cnt = logEntries.Count;
            var prefixHeight = height + (cnt > 0 ? logEntries[cnt - 1].prefixHeight : 0);
            logEntries.Add(new LogEntry(height, prefixHeight, text));
        ...
        private void RefreshEntryHeight()
        {
            ...
            {
                var text = logEntries[i].isSubtitle
                    ? logEntries[i].subtitleText
                    : logEntries[i].displayData.FormatNameDialogue();
                var height = contentForTest.GetPreferredHeight(text, contentDefaultWidth);
        ...
        public void ProvideData(Transform transform, int idx)
        {
            ...
            if (logEntry.isSubtitle)
            {
                // Subtitle entries: grey text, no go-back, no voice replay, no double-click jump.
                logEntryController.InitSubtitle(logEntry.subtitleText, logEntry.height);
                return;
            }
