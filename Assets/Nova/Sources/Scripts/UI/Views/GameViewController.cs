...
namespace Nova
{
    ...
    public class GameViewController : ViewControllerBase, IRestorable
    {
        ...
        private void OnRouteEnded(RouteEndedData routeEndedData)
        {
            ...
            // 死亡结局：不回主界面，切到流程图并停在死亡节点上
            // 通过 DeathRouteRegistry 静态查询（FlowChartView 不在 ViewManager 子层级下，
            // viewManager.GetController 找不到它）
            var deathRouter = DeathRouteRegistry.Current;
            if (deathRouter != null && deathRouter.IsDeathNode(routeEndedData.endName))
            {
                deathRouter.PrepareDeathEnding(routeEndedData.endName);
                this.Hide(() => deathRouter.Show(true, null));
                return;
            }
