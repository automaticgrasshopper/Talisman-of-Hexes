...
namespace Nova
{
    public class LogEntryController : MonoBehaviour
    {
        ...
        // Subtitle entries: bypass DialogueDisplayData, render grey via TMP rich-text,
        // and leave both go-back and play-voice buttons disabled so the entry is non-interactive.
        public void InitSubtitle(string text, float height)
        {
            InitReferences();
            InitButton(goBackButton, null);
            InitButton(playVoiceButton, null);
            this.displayData = null;
            textProxy.text = "<color=#888888>" + text + "</color>";
            this.height = height;
        }
